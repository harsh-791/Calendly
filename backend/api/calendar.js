...
const oauth2Client = new OAuth2Client({
  ...
  redirectUri: "http://localhost:3000/api/auth/callback", // Add this line
});
  ...
  try {
    ...
    if (!userId || !tokenStore.has(userId)) {
      ...
      return res.status(401).json({
        ...
        details: {
          userId: userId,
          tokenExists: tokenStore.has(userId),
        },
      });
    }
    ...
    console.log("User tokens retrieved:", userTokens);
    ...
    console.log("Created calendar client");
    ...
    if (date) {
      ...
      console.log("Fetching all events in range:", {
        start: timeMin,
        end: timeMax,
      });
    }
